action :configure do
  base_instance = node['hc_tomcat']['base_instance']
  ...
  [:catalina_options, :java_options, :use_security_manager, :authbind,
   ...
   :user, :group, :tmp_dir, :lib_dir, :endorsed_dir, :uriencoding].each do |attr|
    unless new_resource.instance_variable_get("@#{attr}")
      new_resource.instance_variable_set("@#{attr}", node['hc_tomcat'][attr])
    end
  end
  ...
  if new_resource.name == 'base'
    ...
    [:base, :home, :config_dir, :source_dir, :log_dir, :work_dir, :context_dir,
     :webapp_dir].each do |attr|
      unless new_resource.instance_variable_get("@#{attr}")
        new_resource.instance_variable_set("@#{attr}", node['hc_tomcat'][attr])
      end
    end
    [:log_dir, :scimweblogs_dir].each do |dir|
      directory  new_resource.instance_variable_get("@#{dir}") do
        mode '2775'
        recursive true
        user 'tomcat'
        group 'tomcat'
    ...
    [:base, :home, :config_dir, :source_dir, :log_dir, :work_dir, :context_dir,
     :webapp_dir].each do |attr|
      if !new_resource.instance_variable_get("@#{attr}") && node['hc_tomcat'][attr]
        new = node['hc_tomcat'][attr].sub(base_instance, instance)
    ...
    %w(catalina.policy catalina.properties context.xml
      ...
      link "#{new_resource.config_dir}/#{file}" do
        to "#{node['hc_tomcat']['config_dir']}/#{file}"
      end
    end
    ...
    if node['init_package'] == 'systemd' && !platform_family?('debian')
      template "/usr/lib/systemd/system/#{instance}.service" do
        source 'product/phix/cp/tomcat.service.erb'
        variables(
          instance: instance,
      ...
      ### Inour case this part is executed so we need to create the init.d service before this to avoid service failure
  ...
  when 'rhel', 'fedora'
    template "/etc/sysconfig/#{instance}" do
      source 'product/phix/cp/sysconfig_tomcat6.erb'
      variables(
        user: new_resource.user,
  ...
  when 'suse'
    template '/etc/tomcat/tomcat.conf' do
      source 'product/phix/cp/sysconfig_tomcat7.erb'
      variables(
        user: new_resource.user,
        home: new_resource.home,
        base: new_resource.base,
  ...
  when 'smartos'
    ...
    template "/etc/default/#{instance}" do
      source 'product/phix/cp/default_tomcat.erb'
      variables(
        user: new_resource.user,
        group: new_resource.group,
        home: new_resource.home,
...
when 'phix/cp'
  template "#{new_resource.config_dir}/server.xml" do
    ...
    source "product/#{new_resource.source_dir}/server.xml.erb"
    variables(
      ...
      connection_timeout: new_resource.connection_timeout,
  ...
  if new_resource.ssl_cert_file.nil?
    execute 'Create Tomcat SSL certificate' do
      group new_resource.group
      command <<-EOH
        #{node['hc_tomcat']['keytool']} \
         -genkey \
         -keystore "#{new_resource.config_dir}/#{new_resource.keystore_file}" \
         -storepass "#{node['hc_tomcat']['keystore_password']}" \
         -keypass "#{node['hc_tomcat']['keystore_password']}" \
         -dname "#{node['hc_tomcat']['certificate_dn']}" \
         -keyalg "RSA"
      EOH
      umask 0007
    ...
    script "create_keystore-#{instance}" do
      ...
      code <<-EOH
        ...
        openssl pkcs12 -export \
         ...
         -chain \
         -CAfile cacerts.pem \
         -password pass:#{node['hc_tomcat']['keystore_password']} \
