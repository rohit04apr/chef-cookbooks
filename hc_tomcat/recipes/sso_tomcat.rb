...
log 'hc_tomcat::default recipe execution started'
...
link "/usr/local/#{node['hc_tomcat']['symlink']}" do
  to "/usr/local/#{node['hc_tomcat']['tomcat_dir_name']}"
end

link "/usr/local/tomcat7" do
  to "/usr/local/#{node['hc_tomcat']['tomcat_dir_name']}"
end
...
if node['hc_tomcat']['run_base_instance']
  hc_tomcat_instance 'base' do
    ...
    proxy_name node['hc_tomcat']['proxyname']
    ...
    scimweblogs_dir node['hc_tomcat']['sso']['scimweblogs_dir']
  end
...
log 'hc_tomcat::default recipe execution finished'
