...
# Recipe:: wellcare_phix
#
# Copyright (c) 2015 The Authors, All Rights Reserved.
#
Chef::Log.info 'hc_tomcat::wellcare_phix recipe execution started'
...
when 'rhel'
  template "/etc/init.d/tomcat#{node['hc_tomcat']['base_version']}" do
    source 'product/phix/wellcare/tomcat.erb'
    mode '0755'
  ...
  template "/etc/init.d/tomcat#{node['hc_tomcat']['base_version']}" do
    source 'product/phix/wellcare/ubuntu.tomcat.service.erb'
    mode '0755'
    owner node['hc_tomcat']['user']
    group node['hc_tomcat']['group']
  end
...
template "#{node['hc_tomcat']['tomcat_home']}/bin/setenv.sh" do
  source 'product/phix/wellcare/setenv.sh.erb'
  mode '0644'
  owner node['hc_tomcat']['user']
  group node['hc_tomcat']['group']
end

%W(#{node['hc_tomcat']['tomcat_home']}/logback /usr/local/phix_log /usr/local/HeapDump).each do |dir|
  directory dir do
    mode '0755'
    owner node['hc_tomcat']['user']
    group node['hc_tomcat']['group']
...
%w(logback.properties logback_de.properties).each do |config|
  template "#{node['hc_tomcat']['tomcat_home']}/logback/#{config}" do
    source "product/phix/wellcare/#{config}.erb"
    mode '0644'
    owner node['hc_tomcat']['user']
    group node['hc_tomcat']['group']
  end
...
remote_file "#{node['hc_tomcat']['tomcat_home']}/lib/ojdbc6.jar" do
  source "http://#{artfct_user_details['username']}:#{artfct_user_details['password']}@artifactory.demo.company.com/artifactory/chef-repo/tomcat/ojdbc6.jar"
  owner 'root'
  group 'root'
  mode '0755'
end
...
execute "chown-tomcat" do
  cwd "#{node['hc_tomcat']['tomcat_home']}/lib"
  command  "chown #{node['hc_tomcat']['user']}:#{node['hc_tomcat']['group']} ojdbc6.jar "
  notifies :restart, "service[#{node['hc_tomcat']['base_instance']}]", :delayed
end

execute "chmod-context.xml" do
  cwd "#{node['hc_tomcat']['tomcat_home']}/conf"
  command  "chmod 640 context.xml"  
end
...
Chef::Log.info 'hc_tomcat::wellcare_phix recipe execution finished'
