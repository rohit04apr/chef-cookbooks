...
#
log 'hc_tomcat::fidelity_phix recipe execution started'
...
when 'rhel'
  template "/etc/init.d/tomcat#{node['hc_tomcat']['base_version']}" do
    source 'product/phix/fidelity/tomcat.erb'
    mode '0755'
  ...
  template "/etc/init.d/tomcat#{node['hc_tomcat']['base_version']}" do
    source 'product/phix/fidelity/ubuntu.tomcat.service.erb'
    mode '0755'
    owner node['hc_tomcat']['user']
    group node['hc_tomcat']['group']
  end
...
%w(heapdump gc_logs app_logs de_app_logs fidelity_app_logs).each do |dir|
...
%w(logback.properties logback_de.properties logback-fidelity.properties).each do |config|
  template "#{node['hc_tomcat']['tomcat_home']}/logback/#{config}" do
    source "product/phix/fidelity/#{config}.erb"
    mode '0644'
    owner node['hc_tomcat']['user']
    group node['hc_tomcat']['group']
  end
...
log 'hc_tomcat::fidelity_phix recipe execution finished'
