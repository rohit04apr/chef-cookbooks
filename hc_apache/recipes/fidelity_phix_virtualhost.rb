...
Chef::Log.info "hc_apache::fidelity_phix_virtualhost started "
Chef::Log.info 'Including recipe hc_apache::default'
include_recipe 'hc_apache::default'

#node[apache]['default_modules']
%w(fidelity_phix.conf external-integration.conf portals.conf).each do |conf|
template "#{node['apache']['dir']}/sites-available/#{conf}" do
  source "product/phix/fidelity/#{conf}.erb"
  mode 0644
  owner 'root'
...
#COMMENT
Chef::Log.info "hc_apache::fidelity_phix_virtualhost finished "
